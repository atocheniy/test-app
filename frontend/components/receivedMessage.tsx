...
export default function receivedMessage ({ Id, Content, Time, Attachments, ReplyTo, AuthorName = "User", onReply }: Message) {
    ...
    return(
        <div className="flex flex-col self-start max-w-[75%] sm:max-w-md my-0.5 group">
            <div className="flex flex-col gap-1 px-3.5 py-2 bg-zinc-950/80 hover:bg-zinc-900/90 transition rounded-2xl rounded-tl-sm border border-white/5 text-zinc-100 shadow-sm min-w-[85px]">
                ...
                <div className="flex items-end justify-between gap-3">
                    {Content && Content.trim().length > 0 && (
                        <p className="text-[14px] leading-relaxed break-words whitespace-pre-wrap text-zinc-300">
                            {Content}
                        </p>
                    )}
                            ...
                            <div className="absolute right-0 bottom-full mb-1.5 w-32 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl py-1 z-50 flex flex-col text-xs text-zinc-300 backdrop-blur-md animate-in fade-in zoom-in-95 duration-100">
                                ...
                                {onReply && (
                                    <button
                                        ...
                                        onClick={() => {
                                            setIsMenuOpen(false);
                                            onReply({ id: Id, content: Content || "", authorName: AuthorName });
                                        }}
